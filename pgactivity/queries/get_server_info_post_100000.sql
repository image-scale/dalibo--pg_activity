-- Server info query for PostgreSQL 10
SELECT
    ...
    (SELECT count(*) FROM pg_stat_activity
        WHERE state = 'idle in transaction (aborted)' AND backend_type = 'client backend'
    ...
    (SELECT count(*) FROM pg_stat_activity WHERE backend_type = 'autovacuum worker')::int AS autovacuum_workers,
    (SELECT setting FROM pg_settings WHERE name = 'autovacuum_max_workers')::int AS autovacuum_max_workers,
    (SELECT count(*) FROM pg_stat_activity WHERE backend_type = 'logical replication worker')::int AS logical_replication_workers,
    (SELECT count(*) FROM pg_stat_activity WHERE backend_type = 'parallel worker')::int AS parallel_workers,
    (SELECT setting FROM pg_settings WHERE name = 'max_logical_replication_workers')::int AS max_logical_replication_workers,
    (SELECT setting FROM pg_settings WHERE name = 'max_parallel_workers')::int AS max_parallel_workers,
    (SELECT setting FROM pg_settings WHERE name = 'max_worker_processes')::int AS max_worker_processes,
    (SELECT setting FROM pg_settings WHERE name = 'max_wal_senders')::int AS max_wal_senders,
    (SELECT setting FROM pg_settings WHERE name = 'max_replication_slots')::int AS max_replication_slots,
